...
flags.DEFINE_bool(
    "use_multi_task", True,
    "Whether to use multi-task, multi-head BLEURT training.")

flags.DEFINE_list(
    "task_types", ["en", "re", "te", "fa"],
    "List of task type codes (e.g., 'en', 're', 'te', 'fa').")

flags.DEFINE_integer("multi_head_width", 128,
                     "Width of hidden layer before each task head.")

flags.DEFINE_float(
    "other_head_loss_weight", 0.1,
    "Weight for auxiliary loss pushing non-active heads toward other_head_target."
)

flags.DEFINE_float(
    "other_head_target", 1.0,
    "Target score for non-active heads (typically 1.0 for 'no change')."
)
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 ...
                 hidden_layers_width, dropout_rate, use_multi_task=False,
                 task_types=None, task_id_tensor=None):
  ...
  Args:
    ...
    use_multi_task: bool. whether to use multi-task heads.
    task_types: list of task type codes (e.g., ['en', 're', 'te', 'fa']).
  ...
  if use_multi_task and task_types:
    # Multi-task: build separate heads for each task
    logging.info("Building multi-task heads for tasks: {}".format(
        ", ".join(task_types)))
    num_tasks = len(task_types)
    task_heads = []
    
    for task_idx, task_type in enumerate(task_types):
      with tf.variable_scope("task_head_{}".format(task_type)):
        # Hidden layer before head
        head_output = tf.layers.dense(
            output_layer, 1, bias_initializer=tf.constant_initializer(0.15),
            name="output")
        head_output = tf.squeeze(head_output, 1)  # [batch_size]
        task_heads.append(head_output)
    
    # Stack all head outputs: [batch_size, num_tasks]
    all_head_outputs = tf.stack(task_heads, axis=1)
    
    # Choose the correct head per example using task_id_tensor
    # all_head_outputs: [batch, num_tasks]
    # predictions (selected head): [batch]
    # labels: typically [batch]

    labels_f = tf.cast(labels, tf.float32)
    labels_f = tf.reshape(labels_f, [-1])  # [batch]

    # Encourage other heads to output ~1.0
    alpha = FLAGS.other_head_loss_weight  # e.g., 0.1
    other_target = FLAGS.other_head_target  # e.g., 1.0

    if task_id_tensor is not None:
      head_indices = tf.cast(task_id_tensor, tf.int32)  # [batch]

      batch_size = tf.shape(input_ids)[0]
      batch_indices = tf.range(batch_size, dtype=tf.int32)
      gather_indices = tf.stack([batch_indices, head_indices], axis=1)
      predictions = tf.gather_nd(all_head_outputs, gather_indices)  # [batch]

      # Mask for non-active heads
      one_hot = tf.one_hot(head_indices, depth=num_tasks, dtype=tf.float32)  # [batch, num_tasks]
      mask_non_active = 1.0 - one_hot                                       # [batch, num_tasks]

      # Aux loss (non-active heads -> other_target), per-example then mean
      mse_non_active = tf.square(all_head_outputs - other_target) * mask_non_active
      per_example_non_active = tf.reduce_sum(mse_non_active, axis=1)         # [batch]
      aux_loss = tf.reduce_mean(per_example_non_active)                      # scalar

      # Active loss
      if FLAGS.use_ranking_loss:
        active_pair_losses = ranking_loss(predictions, labels_f)             # [#pairs]
        active_loss = tf.reduce_mean(active_pair_losses)                     # scalar
        # For logging/return, you can keep pair losses or make a batch-shaped proxy:
        per_example_loss = per_example_non_active  # [batch] (useful to monitor aux pressure)
      else:
        per_example_active = tf.square(predictions - labels_f)               # [batch]
        active_loss = tf.reduce_mean(per_example_active)                     # scalar
        per_example_loss = per_example_active + alpha * per_example_non_active  # [batch]

      loss = active_loss + alpha * aux_loss
      
    else:
      # No task id provided: your current fallback behavior
      predictions = all_head_outputs
      if FLAGS.use_ranking_loss:
        per_example_loss = ranking_loss(predictions, labels_f)
        loss = tf.reduce_mean(per_example_loss)
      else:
        per_example_loss = tf.square(predictions - labels_f)
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    # Extract task type if multi-task mode
    # task_type_tensor = None
    task_id_tensor = None
    if FLAGS.use_multi_task:
      if "task_id" in features:
        task_id_tensor = tf.cast(features["task_id"], tf.int32)
        logging.info("Using task_id from features.")
      else:
        # logging.warning("Multi-task mode enabled but 'task_id' not in features")
        logging.info("No task_id in features (expected for all-heads serving).")
        if (mode != tf_estimator.ModeKeys.PREDICT):
          raise ValueError("Missing task_id in TRAIN/EVAL features.")
    ...
    # Call create_model with multi-task parameters
    model_output = create_model(
        ...
        dropout_rate, use_multi_task=FLAGS.use_multi_task,
        task_types=FLAGS.task_types, task_id_tensor=task_id_tensor)
    
    total_loss, per_example_loss, pred = model_output
    
    tf.logging.info("Total Loss = %s", total_loss)
    tf.logging.info("Global step = %s", tf.train.get_global_step())
...
def input_fn_builder(tfrecord_file,
  ...
  Args:
    tfrecord_file: path(s) to TFRecord files.
    seq_length: sequence length for BERT.
    is_training: bool, whether to use training mode.
    batch_size: batch size.
    drop_remainder: bool, whether to drop incomplete batches.
    task_filter: optional str, filter examples by this task type.
  """
  logging.info(
  ...
  def input_fn(params):  # pylint: disable=unused-argument
    ...
    if task_filter is not None and FLAGS.use_multi_task:
      logging.info("Filtering examples for task: {}".format(task_filter))
      # d = d.filter(lambda example: tf.equal(example["type"], task_filter))
      task_id_py = FLAGS.task_types.index(task_filter)
      const_task_id = tf.constant(task_id_py, dtype=tf.int32)
      d = d.filter(lambda example: tf.equal(example["task_id"], const_task_id))
...
def _serving_input_fn_builder(seq_length):
  ...
  multi_task = bool(FLAGS.task_types)
  ...
  if tf.executing_eagerly():
    ...
    if multi_task:
      # shape [0] to match [batch] task ids
      name_to_features["task_id"] = tf.zeros(dtype=tf.int64, shape=[0])
  else:
    ...
    if multi_task:
      # one task id per example
      name_to_features["task_id"] = tf.placeholder(
          tf.int64, shape=[None], name="task_id")
      
  return tf_estimator.export.build_raw_serving_input_receiver_fn(
      name_to_features)
  
def _serving_input_fn_builder_all_heads(seq_length):
  """Serving input without task_id: returns all head scores."""
  if tf.executing_eagerly():
    name_to_features = {
        "input_ids": tf.zeros(dtype=tf.int64, shape=[0, seq_length]),
        "input_mask": tf.zeros(dtype=tf.int64, shape=[0, seq_length]),
...
def run_finetuning(train_tfrecord,
  ...
  Supports both single-task and multi-task (multi-head) training.
  When use_multi_task=True, creates separate input pipelines per task
  and performs round-robin training by switching tasks each step.
  """

  logging.info("Initializing BLEURT training pipeline.")
  logging.info("Multi-task mode: {}".format(FLAGS.use_multi_task))
  ...
  # For multi-task training: create separate input functions per task
  if FLAGS.use_multi_task and FLAGS.task_types:
    logging.info("Creating multi-task input pipelines for tasks: {}".format(
        ", ".join(FLAGS.task_types)))
    ...
    logging.info("Using single-task input pipeline.")
    train_input_fn = input_fn_builder(
        train_tfrecord,
        seq_length=max_seq_length,
        is_training=True,
  ...
  if isinstance(dev_tfrecord, str):
    ...
    # For eval, optionally create per-task eval functions
    if FLAGS.use_multi_task and FLAGS.task_types:
      logging.info("Creating per-task eval pipelines.")
      # Create base eval function without task filter for overall metrics
  ...
  exporters = [
      ...
      tf_estimator.BestExporter(
          ...
          serving_input_receiver_fn=_serving_input_fn_builder_all_heads(max_seq_length),
          event_file_pattern="eval_default/*.tfevents.*",
          compare_fn=_model_comparator,
          exports_to_keep=3),
  ]
