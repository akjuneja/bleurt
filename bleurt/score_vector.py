...
class EagerPredictor(Predictor):
  ...
  def predict(self, input_dict):
    ...
    # task_id is ALWAYS passed, but may be a sentinel (-1) meaning "all heads"
    task_id = input_dict.get("task_id", -1)  # default -1 => all heads
    task_id = tf.convert_to_tensor(task_id, dtype=tf.int64)
    ...
    predictions = self._bleurt_model_ops(
        ...
        segment_ids=tf.constant(input_dict["segment_ids"],dtype=tf.int64),)["predictions"].numpy()
        # task_id=task_id)["predictions"].numpy()
    return predictions
  ...
  def initialize(self):
    ...
    with self._bleurt_graph.as_default():
      ...
      self._bleurt_ops = bleurt_model_ops(
          ...
          task_id=tf.compat.v1.placeholder(tf.int64, name="task_id")
          )
    ...
    with self._bleurt_graph.as_default():
      bleurt_out = self.session.run(
          self._bleurt_ops, {
              ...
              "task_id:0": input_dict.get("task_id", 0),
          })
